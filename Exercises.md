...
```
Base, b = 7
Digits used in base 7 = 0,1,2,3,4,5,6
...
**Code:**
```
000,001,002,003,004,005,006,010,011,012,013,014,015,016,020
```
...
def closer_point(p, a, b):
    distance_a = (a[0] - p[0])**2 + (a[1] - p[1]) **2
    distance_b = (b[0] - p[0])**2 + (b[1] - p[1]) **2
...
def is_point_on_line_1d(a, b, p):
    if (a>=b and p>=b and p<=a) or (a<=b and p>=a and p<=b):
        return True
    else:
        return False
```
